  ...
  if (isText) {
    ...
    textPairs.forEach(pair => {
      ...
      if (pair.currentText === pair.targetText) {
        ...
        currentCell.style.backgroundColor = pair.currentText ? 'lightcoral' : '';
        targetCell.style.backgroundColor = pair.targetText ? 'lightcoral' : '';
      }
  ...
  currentData.forEach((currentItem, currentIndex) => {
    ...
    for (let i = 0; i < targetData.length; i++) {
      ...
      if (isAria) {
        ...
        if (currentKey === targetKey && (!isMeta || currentValue === targetValue)) {
          matchedTargetIndex = i;
          matchedIndices.add(i);
          break;
        }
    ...
    if (isHeader) {
      ...
      currentCell.innerHTML = formatMeta(currentItem, targetData[matchedTargetIndex]);
      targetCell.innerHTML = matchedTargetIndex !== -1 ? formatMeta(targetData[matchedTargetIndex], currentItem) : '';
      ...
      currentCell.innerHTML = isAria ? formatAria(currentItem, targetData[matchedTargetIndex]) : 
                               (isImage ? formatImage(currentItem) : JSON.stringify(currentItem));
...
function formatMeta(metaItem, targetItem) {
  if (!targetItem) {
    return `{
  ...
  const isContentDifferent = metaItem.contentValue !== targetItem.contentValue;

  return `{
    "tagName": "${metaItem.tagName}",
    "contentValue": "${isContentDifferent ? '<span style="color: red;">' + metaItem.contentValue + '</span>' : metaItem.contentValue}"
  }`;
}

function formatAria(ariaItem) {
  return `{
    "link": "${ariaItem.link}",
    "label": "${ariaItem.label}",
    "target": "${ariaItem.target}"
  }`;
}
